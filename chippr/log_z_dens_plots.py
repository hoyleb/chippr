...
s_stk, w_stk, a_stk, c_stk, d_stk, l_stk = '--', 1.5, 1., 'r', '--', 'Stacked '#[(0, (2, 2))], 'Stacked '
s_map, w_map, a_map, c_map, d_map, l_map = '--', 1.5, 1., 'r', '-.', 'Modes (MAP)'#[(0, (2, 1))], 'Modes '
s_exp, w_exp, a_exp, c_exp, d_exp, l_exp = '--', 1.5, 1., 'r', ':', 'Means '#[(0, (1, 2))], 'Means '
s_mle, w_mle, a_mle, c_mle, d_mle, l_mle = '-', 3., 1., 'b', [(0, (2, 2))], r'\texttt{chippr} Optimization '
s_bfe, w_bfe, a_bfe, c_bfe, d_bfe, l_bfe = '-', 3.5, 1., 'b', [(0, (1, 1))], r'\texttt{chippr} Samples '#actually sample mean
s_smp, w_smp, a_smp, c_smp, d_smp, l_smp = '-', 2., 1., 'k', [(0, (1, 1))], r'\texttt{chippr} Samples '
...
def plot_estimators(info, plot_dir):
    ...
    if 'log_mean_sampled_nz' in info['estimators']:
        ...
        sps_log.fill([info['bin_ends'][0]-2., info['bin_ends'][0]-2., info['bin_ends'][0]-1., info['bin_ends'][0]-1.], [-11., -10, -10., -11.], color=c_bfe, alpha=0.5,
                        linewidth=0., label=l_bfe+lnz+r' $1\sigma$')
        sps_log.fill([info['bin_ends'][0]-2., info['bin_ends'][0]-2., info['bin_ends'][0]-1., info['bin_ends'][0]-1.], [-11., -10, -10., -11.], color=c_bfe, alpha=0.25,
                        linewidth=0., label=l_bfe+lnz+r' $2\sigma$')
