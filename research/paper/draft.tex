...
\log[p(\vec{d}_{i} | z_{i})] =& \log[p(\vec{d}_{i} | z_{i})] + \log[p(z_{i} | 
\vec{d}_{i}, \vec{\phi}^{*})]\\
& - \log[p(z_{i} | \vec{\phi}^{*})] - \log[p(\vec{d}_{i} | z_{i}, 
\vec{\phi}^{*})] + \log[p(\vec{d}_{i} | \vec{\phi}^{*})].
\end{split}
\end{align}
...
\begin{enumerate}
	\item The chosen functional form with parameters $\vec{\phi}$ must be 
capable of describing the true redshift distribution.
