...
def make_true(given_key):
    ...
    if test_info['params']['smooth_truth'] == 1:
        ...
        # true_means = bin_mids
        amps = smooth_func(fine_grid)# f(bin_mids)
        # true_sigmas = bin_difs
        amps = (amps[1:] + amps[:-1]) / 2.
        amps /= np.dot(amps, (fine_grid[1:] - fine_grid[:-1]))
        true_funcs = [chippr.discrete(fine_grid, amps)]
        true_amps = [1.]

        true_dict = {'format': 'discrete', 'grid': fine_grid, 'amps': amps}
    else:
        ...
        true_dict = {'format': 'gauss', 'amps': true_amps, 'means': true_means, 'sigmas': true_sigmas}
